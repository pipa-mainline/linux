...
static void aw88261_dev_set_volume(struct aw_device *aw_dev, unsigned int value)
{
	...
	volume = min((value + vol_desc->init_volume),
		     (unsigned int)AW88261_MUTE_VOL);
...
static void aw88261_dev_mute(struct aw_device *aw_dev, bool is_mute)
{
	if (is_mute) {
		...
		regmap_update_bits(aw_dev->regmap, AW88261_SYSCTRL_REG,
				   ~AW88261_HMUTE_MASK,
				   AW88261_HMUTE_ENABLE_VALUE);
	} else {
		regmap_update_bits(aw_dev->regmap, AW88261_SYSCTRL_REG,
				   ~AW88261_HMUTE_MASK,
				   AW88261_HMUTE_DISABLE_VALUE);
				     ...
				     char **prof_name)
{
	...
	if ((index >= aw_dev->prof_info.count) || (index < 0)) {
		dev_err(aw_dev->dev, "index[%d] overflow count[%d]", index,
...
static int aw88261_dev_fw_update(struct aw88261 *aw88261)
{
	...
	ret = aw88261_dev_get_prof_data(aw_dev, aw_dev->prof_index,
					&prof_index_desc);
	if (ret)
		return ret;
	...
	ret = aw88261_dev_reg_update(aw88261,
				     sec_desc[AW88395_DATA_TYPE_REG].data,
...
static int aw88261_reg_update(struct aw88261 *aw88261, bool force)
{
	...
	if (force) {
		ret = regmap_write(aw_dev->regmap, AW88261_ID_REG,
				   AW88261_SOFT_RESET_VALUE);
		if (ret)
			return ret;
...
static void aw88261_start_pa(struct aw88261 *aw88261)
{
	...
	for (i = 0; i < AW88261_START_RETRIES; i++) {
		...
		if (ret) {
			dev_err(aw88261->aw_pa->dev,
				"aw88261 device start failed. retry = %d", i);
			continue;
		} else {
...
static void aw88261_start(struct aw88261 *aw88261, bool sync_start)
{
	...
	if (sync_start == AW88261_SYNC_START)
		...
		queue_delayed_work(system_wq, &aw88261->start_work,
...
static const struct snd_kcontrol_new aw88261_controls[] = {
	SOC_SINGLE_EXT("PCM Playback Volume", AW88261_SYSCTRL2_REG, 6,
		       AW88261_MUTE_VOL, 0, aw88261_volume_get,
		       aw88261_volume_set),
	...
	SOC_SINGLE_EXT("Volume Ramp Up Step", 0, 0, FADE_TIME_MAX,
		       ...
		       aw88261_set_fade_in_time),
	SOC_SINGLE_EXT("Volume Ramp Down Step", 0, 0, FADE_TIME_MAX,
		       FADE_TIME_MIN, aw88261_get_fade_out_time,
		       aw88261_set_fade_out_time),
			...
			struct regmap *regmap)
{
	...
	if (ret) {
		dev_err(&i2c->dev, "%s read chipid error. ret = %d", __func__,
			ret);
		return ret;
	}
...
static int aw88261_i2c_probe(struct i2c_client *i2c)
{
	...
	ret = devm_snd_soc_register_component(&i2c->dev, &soc_codec_dev_aw88261,
					      aw88261_dai,
...
static const struct i2c_device_id aw88261_i2c_id[] = { { AW88261_I2C_NAME },
						       {} };
